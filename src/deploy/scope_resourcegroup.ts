...
export async function DeployResourceGroupScope(azPath: string, resourceGroupName: string, template: string, deploymentMode: string, deploymentName: string, parameters: string, failOnStdErr: Boolean, excludeChangeTypes: String): Promise<Outputs> {
    ...
    const azDeployParameters = [
        ...
        template ?
            ...
            : undefined,
        deploymentMode && deploymentMode != "validate" ? (deploymentMode == "what-if" ? undefined : `--mode ${deploymentMode}`) : "--mode Incremental",
